...
1 MB = 1024 KB
1 GB = 1024 MB
Примечание 4. Files. Указанный архив доступен по ссылке https://stepik.org/media/attachments/lesson/547172/desktop.zip
"""
...
with ZipFile('desktop.zip') as file:
    ...
    for i in i:
        if not i.is_dir():
            print(i.filename.count('/') * '  ' + i.filename.split('/')[-1], get_zize(i.file_size))
        else:
            print('  ' * (i.filename.count('/') - 1) + i.filename.split('/')[i.filename.count('/') - 1])